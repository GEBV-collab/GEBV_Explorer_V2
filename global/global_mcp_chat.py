...
5. **compute_smith_hazel_index** - Rank lines using the Smith-Hazel selection index.
   Takes trait_weights (dict, e.g. {{"GEBV_yield_y": 2.0, "GEBV_Brix_x": 1.0}})
...
- Use **compute_smith_hazel_index** when the user wants to rank across multiple traits
...
- When computing an index, briefly explain that results are ranked by a composite score that accounts for trait correlations"""
