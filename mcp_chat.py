...
5. **compute_smith_hazel_index** - Rank lines using the Smith-Hazel selection index.
   Takes trait_weights (dict of trait name to economic weight, e.g. {{"GEBV_yield": 2.0, "GEBV_Brix": 1.0}})
   and optional top_n (default 20). Requires at least 2 traits.
   The Smith-Hazel index accounts for genetic correlations between traits,
   so the final ranking reflects how traits co-vary, not just their independent values.
   The page will update automatically to show the ranked results.
...
- Use **compute_smith_hazel_index** when the user wants to rank or optimise across multiple traits
...
- When computing an index, briefly explain that the results are ranked by a composite score
  that accounts for trait correlations, and mention the weights that were applied"""
